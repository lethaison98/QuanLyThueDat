...
namespace QuanLyThueDat.Application.Service
{
    public class ThongBaoDonGiaThueDatService : IThongBaoDonGiaThueDatService
    {
        ...
        public ThongBaoDonGiaThueDatService(QuanLyThueDatDbContext context, IHttpContextAccessor HttpContextAccessor, IQuyetDinhThueDatService QuyetDinhThueDatService)
        {
            _context = context;
        ...
        public async Task<ApiResult<int>> InsertUpdate(ThongBaoDonGiaThueDatRequest rq)
        {
            ...
            {
                item.IdTaiLieu = entity.IdThongBaoDonGiaThueDat;
                item.IdLoaiTaiLieu = NhomLoaiTaiLieuConstant.NhomThongBaoDonGiaThueDat;
                item.TrangThai = 1;
        ...
        public async Task<ApiResult<List<ThongBaoDonGiaThueDatViewModel>>> GetAll()
        {
            ...
            var data = await _context.ThongBaoDonGiaThueDat.Include(x => x.DoanhNghiep).Where(x => !x.IsDeleted).ToListAsync();
        ...
        public async Task<ApiResult<PageViewModel<ThongBaoDonGiaThueDatViewModel>>> GetAllPaging(int? idDoanhNghiep, string keyword, int pageIndex, int pageSize)
        {
            ...
            foreach (var entity in data)
            {
                ...
                {
                    thongBaoDonGiaThueDat.QuyenDuLieu = _quyetDinhThueDatService.CheckQuyenDuLieu(entity.IdQuyetDinhThueDat.Value);
                }
                var listFileViewModel = new List<FileTaiLieuViewModel>();
                var listFile = _context.FileTaiLieu.Include(x => x.File).Where(x => x.IdLoaiTaiLieu == NhomLoaiTaiLieuConstant.NhomThongBaoDonGiaThueDat && x.IdTaiLieu == entity.IdThongBaoDonGiaThueDat && x.TrangThai != 4).ToList();
                ...
                thongBaoDonGiaThueDat.DsFileTaiLieu = listFileViewModel;
        ...
        {
            var result = new List<ThongBaoDonGiaThueDatViewModel>();
            var data = await _context.ThongBaoDonGiaThueDat.Include(x => x.DoanhNghiep).Where(x => !x.IsDeleted).Where(x=> (DateTime.Now.Date.AddDays(90) > x.NgayHetHieuLucDonGiaThueDat) && (DateTime.Now < x.NgayHetHieuLucDonGiaThueDat)).ToListAsync();
            foreach (var item in data)
            {
