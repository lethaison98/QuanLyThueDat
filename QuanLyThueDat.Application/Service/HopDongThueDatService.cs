...
namespace QuanLyThueDat.Application.Service
{
    public class HopDongThueDatService : IHopDongThueDatService
    {
        ...
        public HopDongThueDatService(QuanLyThueDatDbContext context, IHttpContextAccessor HttpContextAccessor, IQuyetDinhThueDatService QuyetDinhThueDatService)
        {
            _context = context;
        ...
        public async Task<ApiResult<int>> InsertUpdate(HopDongThueDatRequest rq)
        {
            ...
            if (entity == null)
            {
                entity = new HopDongThueDat()
                {
                    ...
                    NguoiTao = tenUser,
                    IdNguoiTao = userId,
                    SoHopDongDieuChinh = rq.SoHopDongDieuChinh, 
                    GhiChu = rq.GhiChu,
                };
            }
            else
        ...
        public async Task<ApiResult<PageViewModel<HopDongThueDatViewModel>>> GetAllPaging(int? idDoanhNghiep, string keyword, int pageIndex, int pageSize)
        {
            ...
            foreach (var entity in data)
            {
                ...
                {
                    hopDongThueDat.QuyenDuLieu = _quyetDinhThueDatService.CheckQuyenDuLieu(entity.IdQuyetDinhThueDat.Value);
                }
